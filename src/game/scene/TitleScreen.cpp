...
void TitleScreen::render(Renderer& renderer) {
	...
	for (int i = 0; i < (int)menuItems_.size(); ++i) {
		...
		const glm::vec3 color =
		  selected ? glm::vec3(1.0f, 1.0f, 0.2f) : glm::vec3(0.7f, 0.7f, 0.7f);
	...
	renderer.drawTextHUD(*font_, "↑↓ で選択  Enter で決定",
	                     glm::vec2(cx - 140.0f, windowHeight_ - 30.0f), 0.8f,
...
void TitleScreen::onSelect(SceneManager& sm) {
	...
	case 0: // スタート
		sm.switchTo(std::make_unique<GameScene>(sm.getWindowWidth(),
		                                        sm.getWindowHeight()));
		break;
	case 1: // おわり
		std::exit(0);
